...
class MettlerToledoDevice(object):
    ...
    def beep(self):
        ...
        beep
        '''
        response = self._send_request_get_response('M12 0')
        if 'I' in response[1]:
            raise MettlerToledoError('No beep.')
        return response[2:]
    ...
    def command(self, command):
        ...
        '''
        response = self._send_request_get_response(command)
        if 'I' in response[1]:
            raise MettlerToledoError('Command not accepted')
        time.sleep(1)
        return response[2:]
